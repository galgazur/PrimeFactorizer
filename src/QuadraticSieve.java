...
public class QuadraticSieve {
	 ...
	 * @return
	 */
	public BigInteger factorize(long SmoothnessBound, int linearIndependenceLimit, PrimeFinder firstXprimes,
			BigInteger N, long timeouts) {
		...
		long[][] qxEVectorsArray = new long[eVectorCount][factorBase.length + 1]; // contains
																					// e-vectors
																					// for
																					// each
																					// q(x)
		...
		for (long x = 0; x < Long.MAX_VALUE - 5; x++) {
			...
			for (long i = 1; i >= -1; i -= 2) {
				...
				long[] factorization = smoothnessAndExponentVectors(i * x, factorBase); // first
																						// byte
																						// is
																						// not
																						// part
		...
		bitCombo = (long) Math.pow(2, qxEVectorsArray.length);
	...
	/**
	 * Factorize a given number x and determine if it is smooth with regards to
	 * the factorbase. return an array of factors if it is, and null if not
	 * 
	 * @param qX
	 * @param factorBase
	 * @return
	...
	private long[] smoothnessAndExponentVectors(long x, long[] factorBase) {
		...
		if (isSmooth) {
			//
			ArrayList<Long> factors = smoothness.getFactors(); // 7 11 13 19 19
																// 19 23 37 47
			if (factors.size() == 0) {
				return null;
			}
			for (int i = 0; i < factorBase.length; i++) { // 5 7 11 13 17 19 23
															// 29 31 47 53
	...
	/**
	 * Given all exponentVectors, find a linearly dependant combination of
	 * vectors
	 * 
	 * @param qxFactorArray
	 * @param start
	...
	private long findTCombination(long[][] qxFactorArray, long start) {
		...
		for (long i = start + 1; i < bitCombo; i++) {
			...
			// }
			isValidCombo = true;
			int isOk = 0; // denotes if the following eVector should be included
							// or if it includes too many eVectors, or includes
							// j3
			...
			if ((isOk <= linearIndependenceLimit)) {// &&(x)){
				for (int j = 1; j < factorBase.length + 1; j++) { // for every
																	// bit
					bitSum = 0;// qxFactorArray[j3][j];
					for (int j2 = 0; j2 < qxFactorArray.length; j2++) { // for
																		// every
																		// x
